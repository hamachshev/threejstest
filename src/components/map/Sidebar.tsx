...
let itemTypeLabels: Record<Item["type"], string> = {
  cube: "Cubes",
  bin: "Bins",
};
...
let Sidebar = ({
  ...
  let [pendingImportKind, setPendingImportKind] =
    useState<importKind>("floorSvg");
  ...
  return (
    ...
    >
      ...
      <button onClick={onAddCube} disabled={!editing}>
        Cube
      </button>
      <button onClick={onAddBin} disabled={!editing}>
        Bin
      </button>
      ...
      />
      <button onClick={() => openFilePicker("floorSvg")}>
        Import Floor SVG
      </button>
      <button
