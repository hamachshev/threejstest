...
export let worldToScreen = (p: WorldPoint) => screenPoint(CENTER + p.x * UNIT_SIZE, CENTER + p.y * UNIT_SIZE)
export let screenToWorld = (p: ScreenPoint) => worldPoint((p.x - CENTER) / UNIT_SIZE, (p.y - CENTER) / UNIT_SIZE)

export let distance = <T extends Point>(a: T, b: T) => Math.hypot(b.x - a.x, b.y - a.y)
...
export let distanceToSegment = (p: WorldPoint, a: WorldPoint, b: WorldPoint) => {
	...
	let t = Math.max(0, Math.min(1, ((p.x - a.x) * dx + (p.y - a.y) * dy) / lengthSq))
	let closest = worldPoint(a.x + t * dx, a.y + t * dy)
	return distance(p, closest)
}

export let computeSegments = (points: WorldPoint[], closed: boolean): Segment[] => {
...
export let resizeSegment = (points: WorldPoint[], index: number, newLength: number): WorldPoint[] | null => {
