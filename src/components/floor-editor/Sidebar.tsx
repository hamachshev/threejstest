...
export let Sidebar = ({
	...
	return (
		<div style={{ width: 220, padding: 10, background: "#eee", overflowY: "auto" }}>
			<h3 style={{ marginTop: 0 }}>Floor Editor</h3>
			<p style={{ fontSize: 12, color: "#555" }}>
				{closed ? "Click a segment to edit its length, or drag a vertex to reshape." : "Click to place points. Click the first point again (or press Close Shape) to finish."}
			</p>
			<div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
				<button onClick={onUndo} disabled={closed || points.length === 0}>Undo Point</button>
				<button onClick={onCloseShape} disabled={closed || points.length < 3}>Close Shape</button>
				<button onClick={onClear} disabled={points.length === 0}>Clear</button>
			</div>
			<div style={{ marginBottom: 10 }}>
				<button onClick={onExport} disabled={points.length < 3}>Export SVG</button>
			</div>

			{segments.length > 0 && (
				<div style={{ marginBottom: 10 }}>
					<div style={{ fontWeight: "bold", fontSize: 12, marginBottom: 4 }}>Segments</div>
