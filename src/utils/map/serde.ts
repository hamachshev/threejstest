...
export let onExport = async (sceneRef: RefObject<Scene | null>) => {
  ...
  try {
    ...
    let taggedObjects = scene.children.filter(
      (child) =>
        isItemType(child.userData.type) || child.userData.type === "floor",
    );
    let data = (await exporter.parseAsync(taggedObjects, {
      binary: true,
    })) as ArrayBuffer;
...
export let onImport = async (file: File): Promise<ImportResult | null> => {
  try {
    ...
    if (floorChild && !isFloorPolygon(floorChild.userData.floorPolygon))
      return null;

    let itemChildren = children.filter((child) => child !== floorChild);
    if (!itemChildren.every((child) => isItemType(child.userData.type)))
      return null;
