...
export let pointInPolygon = (x: number, z: number, polygon: FloorPoint[]) => {
	...
	for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
		let a = polygon[i]
		let b = polygon[j]
		let intersect = (a.z > z) !== (b.z > z) && x < ((b.x - a.x) * (z - a.z)) / (b.z - a.z) + a.x
...
export let segmentsIntersect = (p1: FloorPoint, p2: FloorPoint, p3: FloorPoint, p4: FloorPoint) => {
...
export let footprintInPolygon = (x: number, z: number, halfX: number, halfZ: number, polygon: FloorPoint[]) => {
	...
	if (!corners.every((corner) => pointInPolygon(corner.x, corner.z, polygon))) return false
